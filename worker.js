...
async function handleRequest(request) {
  ...
  // CHECK FIRST: Is this a web.cloudmoonapp.com request? (native support)
  if (url.pathname.startsWith('/web.cloudmoonapp.com')) {
    return proxyCloudMoon(request);
  }
  
  // Otherwise: Universal proxy mode
  const targetURL = parseUniversalURL(url.pathname, url.search);
  ...
  // Otherwise return shadow DOM wrapper for universal proxy
