// Cloudflare Worker - Hybrid Proxy (CloudMoon Native + Universal Reverse Proxy)
...
async function handleRequest(request) {
  ...
  // Check if this is a web.cloudmoonapp.com request - use native CloudMoon handler
  if (url.pathname.startsWith('/web.cloudmoonapp.com') || url.pathname === '/web.cloudmoonapp.com/') {
    return proxyCloudMoon(request);
  }
  
  // Parse for universal proxy
  const targetURL = parseTargetURL(url.pathname, url.search);
  
  if (!targetURL || !isValidURL(targetURL)) {
    return new Response('Invalid URL', { status: 400 });
  }
  
  // If target is CloudMoon, use native handler
...
// NATIVE CLOUDMOON HANDLER (exact original logic)
